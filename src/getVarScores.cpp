...
int read_all_data(par_info *pi,sa_par_info *spi,subject **sub,int *nsubptr,char names[MAX_LOCI][20],char comments[MAX_LOCI][MAX_COMMENT_LENGTH],float func_weight[MAX_LOCI])
{
	...
	// if code below works OK, move into scoreassoc.cpp
	if (spi->df[LOCUSFILTERFILE].fp)
	{
		pi->n_loci_to_use=0;
		for (l=0;fscanf(spi->df[LOCUSFILTERFILE].fp," %d",&use)==1;++l)
		{
			if (use==1)
		...
		{
			dcerror(1,"Number of values in locusfilterfile %s (%d) does not match --numloci (%d)\n",spi->df[LOCUSFILTERFILE].fn,pi->nloci); exit(1);
		}
	}
	else
	{
		if (pi->nloci==0)
		{
			dcerror(1,"Number of loci is 0. Need to either set --numloci or use --locusfilterfile when working with --gcdatafile\n"); exit(1);
		}
